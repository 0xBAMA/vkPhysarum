...
float blurResult ( vec2 uv ) {
	...
	for ( float offset = -3.0f * GlobalData.radius - 0.5f; offset < 3.0f * GlobalData.radius + 0.5f; offset++ )
		...
		val += blurWeight( offset ) * texture( scratchBuffer, ( uv + ( vec2( 0.0f, offset ) / textureSize( scratchBuffer, 0 ).xy ) ) ).r;

	// correction factor
	return val * 0.5f * GlobalData.decayRate;
}

void main () {
	vec2 p = vec2( gl_GlobalInvocationID.xy + 0.5f ) / imageSize( stateBuffer ).xy;
	imageStore( stateBuffer, ivec2( gl_GlobalInvocationID.xy ), vec4( blurResult( p ) ) );
}